...
import { getServersWithStatus } from "../../lib/actions/get-servers";
import type { Server as PrismaServer } from "@prisma/client";

// Extend Server to include runtime status and optional display fields
type Server = PrismaServer & {
    ...
    startTransition(async () => {
      ...
      if (server) {
        ...
        if (newStatus === "Starting") {
          setTimeout(() => {
            setServers(current => current.map(s => s.id === serverId ? { ...s, status: "Online" } : s));
          }, 2000);
        }
      }
      ...
      toast({
        title: "Action Sent",
        description: `The "${action}" command has been sent. (Mock)`,
      });
    });
  };
      ...
      <div className="flex justify-end mb-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="sm:max-w-lg">
            <CreateServerForm closeDialog={() => setOpen(false)} nodes={[]} />
          </DialogContent>
        </Dialog>
      </div>
            ...
            {servers.length > 0 ? (
              servers.map(server => (
                <TableRow key={server.id} className={isPending ? "opacity-50" : ""}>
                  ...
                  <TableCell>{`${server.currentPlayers ?? 0} / ${server.maxPlayers ?? 0}`}</TableCell>
